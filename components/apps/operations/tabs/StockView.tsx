...
const StockView = ({ operations, search, onViewOrder, onQuickAction }: StockViewProps) => {
    const [page, setPage] = useState(1);

    const filtered = useMemo(() => {
        const ops = search(operations);
        // Sort by days in stock (oldest first = highest priority)
        ...
        <div className="rounded-xl border border-slate-200/60 bg-white shadow-sm dark:border-slate-700/40 dark:bg-[#1a2234]">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3 dark:border-slate-700/40">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-sm font-bold text-slate-700 dark:text-white">En Stock</h3>
                    <span className="rounded-md bg-blue-100 px-2 py-0.5 text-xs font-bold text-blue-600 dark:bg-blue-500/15 dark:text-blue-400">{filtered.length}</span>
                </div>
            </div>
            ...
            {filtered.length === 0 ? (
                <div className="flex items-center justify-center py-16">
                    <p className="text-sm text-slate-400">Aucun article en stock</p>
                </div>
            ) : (
                <>
                    <div className="overflow-x-auto">
                        <table className="w-full text-left text-sm">
                            <thead className="border-b border-slate-100 bg-slate-50/50 dark:border-slate-700/40 dark:bg-slate-800/30">
                                <tr>
                                    <th className="px-4 py-3 font-semibold text-slate-600 dark:text-slate-300">Date récup.</th>
                                    <th className="px-4 py-3 font-semibold text-slate-600 dark:text-slate-300">Jours</th>
                            ...
                            <tbody className="divide-y divide-slate-50 dark:divide-slate-800/50">
                                {paged.map((op, i) => {
                                    const days = getDaysInStock(op.date);
                                    return (
