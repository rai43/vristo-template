...
interface DeliveriesViewProps {
    ...
    subView: 'inProgress' | 'completed';
    onSubViewChange: (_view: 'inProgress' | 'completed') => void;
...
function buildDeliveriesWhatsApp(ops: Operation[]): string {
    ...
    lines.push('');
    lines.push(`📊 Total: ${ops.length} livraison${ops.length !== 1 ? 's' : ''}`);
    return lines.join('\n');
}
...
const CopyWhatsAppButton = ({ ops }: { ops: Operation[] }) => {
    ...
    const handleCopy = async () => {
        const text = buildDeliveriesWhatsApp(ops);
        try {
...
const DeliveriesView = ({ operations, search, onViewOrder, subView, onSubViewChange, onQuickAction }: DeliveriesViewProps) => {
    const [page, setPage] = useState(1);
    const [statusFilter, setStatusFilter] = useState<string>('all');

    const filtered = useMemo(() => {
        let ops = search(operations);
        // Filter by subView (in progress vs completed)
        if (subView === 'completed') {
            ops = ops.filter((op) => TERMINAL_STATUSES.has(op.status));
        } else {
            // En cours: show all non-terminal delivery operations
            ops = ops.filter((op) => !TERMINAL_STATUSES.has(op.status));
        }
    ...
    }, [operations, search, subView, statusFilter]);

    const totalPages = Math.ceil(filtered.length / PAGE_SIZE);
    const paged = useMemo(() => filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE), [filtered, page]);

    // Reset page when filters change
    React.useEffect(() => setPage(1), [subView, statusFilter]);
    ...
    const statusOptions =
        subView === 'completed'
            ...
            : [
                  ...
                  { value: 'confirmed', label: '✅ Confirmé (client)' },
        ...
        <div className="rounded-xl border border-slate-200/60 bg-white shadow-sm dark:border-slate-700/40 dark:bg-[#1a2234]">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-3 dark:border-slate-700/40">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-sm font-bold text-slate-700 dark:text-white">Livraisons</h3>
                    <span className="rounded-md bg-emerald-100 px-2 py-0.5 text-xs font-bold text-emerald-600 dark:bg-emerald-500/15 dark:text-emerald-400">{filtered.length}</span>
                </div>
                ...
                <CopyWhatsAppButton ops={filtered} />
                    ...
                    <div className="flex rounded-lg border border-slate-200 dark:border-slate-600">
                        <button
                            type="button"
                            onClick={() => onSubViewChange('inProgress')}
                            className={`px-3 py-1.5 text-xs font-medium transition-colors ${
                                subView === 'inProgress' ? 'bg-emerald-500 text-white' : 'text-slate-600 hover:bg-slate-50 dark:text-slate-300'
                            }`}
                        >
                            En cours
                        </button>
                        <button
                            type="button"
                            onClick={() => onSubViewChange('completed')}
                            className={`px-3 py-1.5 text-xs font-medium transition-colors ${
                                subView === 'completed' ? 'bg-emerald-500 text-white' : 'text-slate-600 hover:bg-slate-50 dark:text-slate-300'
                            }`}
                        >
                            Terminées
                        </button>
            ...
            {filtered.length === 0 ? (
                <div className="flex items-center justify-center py-16">
                    <p className="text-sm text-slate-400">Aucune livraison</p>
                </div>
            ) : (
                <>
                            ...
                            <tbody className="divide-y divide-slate-50 dark:divide-slate-800/50">
                                {paged.map((op, i) => (
                                    <tr key={`${op.orderId}-${op.operationIndex}-${i}`} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                                        ...
                                        <td className="px-4 py-3 text-xs text-slate-500 dark:text-slate-400">{op.deliveryAgent || '-'}</td>
                                        <td className="px-4 py-3">
